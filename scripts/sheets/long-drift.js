...
import { LongDriftActorSheet } from "./actor-sheet.js";
...
Hooks.once("init", () => {
  ...
  try {
    ...
    console.log('long-drift | Handlebars helper "or" registered');
  } catch (e) {
  ...
  loadTemplates([
    ...
    "systems/long-drift/templates/actor/npc-sheet.hbs"
  ...
  DSC.registerSheet(Actor, "long-drift", LongDriftActorSheet, {
...
Hooks.on("updateActor", (actor, changes, options, userId) => {
  ...
  Object.values(ui.windows).forEach(app => {
    if (app.constructor.name === "LongDriftActorSheet" && app.rendered && app.actor) {
...
Hooks.on("updateToken", (tokenDocument, changes, options, userId) => {
  ...
  if (changes.actorData || (changes.delta && Object.keys(changes.delta).length > 0)) {
    ...
    if (baseActor) {
      ...
      Object.values(ui.windows).forEach(app => {
        if (app.constructor.name === "LongDriftActorSheet" && app.rendered) {
  ...
  if (tokenDocument.actorLink && tokenDocument.actor) {
    ...
    Object.values(ui.windows).forEach(app => {
      if (app.constructor.name === "LongDriftActorSheet" && app.actor?.id === actor.id && app.rendered) {
...
Hooks.on("updateTokenActor", (tokenActor, changes, options, userId) => {
  ...
  Object.values(ui.windows).forEach(app => {
    if (app.constructor.name === "LongDriftActorSheet" && app.actor?.id === tokenActor.id && app.rendered) {
