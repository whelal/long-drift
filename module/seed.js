...
function normaliseCyberwareCompendiumEntry(entry) {
  ...
  clone.system = {
    ...
    slotsUsed: Number.isFinite(Number(system.slotsUsed)) ? Math.max(0, Number(system.slotsUsed)) : 1,
    optionSlots: Number.isFinite(Number(system.optionSlots)) ? Math.max(0, Number(system.optionSlots)) : 0,
    isOption: Boolean(system.isOption ?? false),
    requiresFoundation: String(system.requiresFoundation || ""),
